#!/usr/bin/env python3
"""
생성 시간: 2025-08-09 10:42:00 KST
핵심 내용: Part 3 Maintainability 리프 노드의 텍스트 경계를 기반으로 개별 MD 파일 추출
상세 내용:
  ...
  - process_part3_leaf_nodes (라인 101-130): Part 3 전체 처리
상태: 활성
주소: extract_part3_leaf_content
참조: part3_maintainability_leaf_nodes_with_boundaries.json, Part_03_Part_3_Maintainability.md
"""
...
def extract_node_content(md_content, start_text, end_text):
    ...
    if start_pos == -1:
        return f"<!-- 시작 텍스트를 찾을 수 없음: {start_text[:50]}... -->\n"
...
def process_part3_leaf_nodes():
    """Part 3 리프 노드들을 처리하여 개별 MD 파일로 저장"""
    base_dir = '/home/nadle/projects/Knowledge_Sherpa/v2/25-08-09'
    
    # 입력 파일들
    boundaries_file = os.path.join(base_dir, 'part3_maintainability_leaf_nodes_with_boundaries.json')
    md_file = '/home/nadle/projects/Knowledge_Sherpa/v2/25-08-07/extracted_parts/Part_03_Part_3_Maintainability.md'
    
    # 출력 폴더 생성
    output_dir = create_output_folder(base_dir, 'part3_maintainability_leaf_nodes')
    
    # 데이터 로드
    nodes = load_json_data(boundaries_file)
    md_content = read_md_file(md_file)
    
    print(f"Processing Part 3 Maintainability - {len(nodes)} leaf nodes...")
