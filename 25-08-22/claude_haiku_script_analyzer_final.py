...
핵심 내용: Claude Code SDK ClaudeSDKClient를 사용해 Haiku 3.5 모델로 스크립트 파일의 핵심 내용을 추출하는 도구 (최종버전)
상세 내용:
    - main() (라인 84-102): 메인 실행 함수, 인자 처리 및 전체 프로세스 관리
    - analyze_script_with_haiku() (라인 32-79): ClaudeSDKClient를 사용한 비동기 스크립트 분석 함수 (max_tokens 조정)
    - setup_argument_parser() (라인 15-29): 명령행 인자 파싱 설정
    - 클래스 및 상수 정의 (라인 1-13): 필수 임포트 및 설정
상태: active
주소: claude_haiku_script_analyzer/final
참조: claude_haiku_script_analyzer/v5
"""
    ...
    try:
        async with ClaudeSDKClient(
            options=ClaudeCodeOptions(
                ...
                max_turns=1,
                max_tokens=3500  # Haiku 3.5는 최대 4096 토큰, 여유분 고려해서 3500으로 설정
            )
        ) as client:
...
async def main():
    ...
    print("🔍 Claude Code Haiku 3.5 스크립트 분석기 (최종버전)")
    print("=" * 60)
