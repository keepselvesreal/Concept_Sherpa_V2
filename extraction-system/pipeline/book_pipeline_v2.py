...
# 핵심 내용: PDF/EPUB → 통합 노드 문서 생성 파이프라인 (5단계: 목차추출 → 장별폴더 → 콘텐츠노드분석 → 콘텐츠파일추출 → 노드정보문서생성)
# 상세 내용:
#   - BookPipeline (라인 15-550): 메인 파이프라인 클래스 
#   - extract_toc_from_pdf (라인 50-79): PDF 북마크 → 목차 JSON 추출
#   - create_chapters_folders (라인 80-212): 목차 → 장별 폴더 생성
#   - analyze_content_nodes (라인 213-303): 콘텐츠 노드 분석 (has_content 추가)
#   - extract_content_files (라인 304-397): 실제 콘텐츠 파일 추출
#   - generate_node_info_documents (라인 398-480): 노드 정보 문서 생성 (5단계)
#   - execute (라인 481-540): 파이프라인 실행 메인 메서드
#   - PipelineResult (라인 33-42): 결과 반환 클래스
# 상태: active
# 주소: book_pipeline/v2
# 참조: book_pipeline, node_document_generator_v2.py 로직 통합
...
class BookPipeline:
    ...
    def generate_node_info_documents(self, chapters_data: Dict[str, Any]) -> Dict[str, Any]:
        """모든 장별 폴더에 노드 정보 문서들을 생성합니다 (5단계)"""
        try:
            print("📄 === 5단계: 노드 정보 문서 생성 시작 ===")
            
            # NodeDocumentGenerator를 사용하여 문서 생성
            result = self.node_document_generator.generate_documents_for_multiple_chapters(chapters_data)
            
            if result.get('success', False):
                print(f"\n🎉 노드 정보 문서 생성 완료!")
                print(f"📊 성공한 장: {result.get('processed_chapters', 0)}개")
                print(f"📄 생성된 문서: {result.get('total_documents_created', 0)}개")
            else:
                print(f"❌ 노드 정보 문서 생성 실패: {result.get('error', '알 수 없는 오류')}")
