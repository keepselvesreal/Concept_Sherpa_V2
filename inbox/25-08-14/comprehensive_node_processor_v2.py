# 생성 시간: Thu Aug 14 11:03:48 KST 2025
# 핵심 내용: 통일된 파일명 규칙으로 노드 문서를 생성하는 개선된 종합 노드 처리기
# 상세 내용:
#   - load_input_data() (line 29): JSON 노드 정보 또는 마크다운 파일을 입력으로 받아 처리
#   - extract_headers_from_md() (line 68): 마크다운 파일에서 헤더 구조 추출
#   - build_hierarchy() (line 101): 레벨 기반 부모-자식 관계 구축
#   - determine_has_content() (line 145): 상위-하위 노드 사이 내용 존재 여부 판단
#   - save_nodes_json() (line 174): 전체 노드 구조를 nodes.json으로 저장
#   - filter_and_save_content_nodes() (line 194): has_content=true 또는 리프 노드를 content_nodes.json으로 저장
#   - create_node_documents_v2() (line 230): 새로운 파일명 규칙으로 노드 문서 생성 (lev{level}_{id:02d}_{title}_info.md)
#   - process_nodes_comprehensive() (line 297): 전체 처리 프로세스 통합 실행
#   - main() (line 366): CLI 인터페이스 및 실행 로직
# 상태: 활성
# 주소: comprehensive_node_processor_v2
# 참조: comprehensive_node_processor (수정 버전)
...
def create_node_documents_v2(nodes: List[Dict[str, Any]], output_dir: str = "node_docs_v2") -> bool:
    """
    통일된 파일명 규칙으로 노드 문서 생성: lev{level}_{id:02d}_{title}_info.md
    ...
    Returns:
        생성 성공 여부
    """
    print("📄 노드 문서 생성 중 (v2 - 통일된 파일명 규칙)...")
    ...
    for node in nodes:
        try:
            ...
            # 새로운 파일명 규칙: lev{level}_{id:02d}_{title}_info.md
            ...
            if node.get('children_ids'):
                for child_id in node['children_ids']:
                    ...
                    if child_node:
                        ...
                        child_safe_title = re.sub(r'[-\s]+', '_', child_safe_title)
                        child_safe_title = child_safe_title.strip('_').lower()
...
def process_nodes_comprehensive_v2(input_path: str, output_dir: str = ".") -> bool:
    """
    전체 노드 처리 프로세스를 실행합니다 (v2 - 새로운 파일명 규칙).
    ...
    try:
        ...
        # 6. 노드 문서 생성 (v2 - 새로운 파일명 규칙)
        print("\n6️⃣ 노드 문서 생성 (v2)")
        docs_dir = os.path.join(output_dir, "node_docs_v2")
        create_node_documents_v2(nodes, docs_dir)
        ...
        print(f"     • {docs_dir}/ (새 파일명 규칙: lev{'{level}'}_{'{id:02d}'}_{'{title}'}_info.md)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='통일된 파일명 규칙으로 노드 정보를 종합 처리 (v2)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
사용 예시:
  python comprehensive_node_processor_v2.py document.md
  python comprehensive_node_processor_v2.py nodes.json -o ./output
  python comprehensive_node_processor_v2.py script_structure.json --output-dir ./processed

새로운 파일명 규칙:
  lev{level}_{id:02d}_{title}_info.md
  예: lev1_01_introduction_and_overview_info.md
        """
    )
    ...
    # 처리 실행
    success = process_nodes_comprehensive_v2(args.input_file, args.output_dir)
