...
핵심 내용: MD 파일에서 첫 번째 # 헤더를 추출하여 JSON 파일 생성하는 스크립트
상세 내용:
    - extract_first_header(md_file_path: str) -> str: MD 파일에서 첫 번째 # 헤더의 텍스트 추출
    - create_json_from_md(md_file_path: str) -> None: JSON 파일 생성 (id: 0, title: 추출된 헤더, level: 0)
    - main(): 명령행 인자 처리 및 실행
상태: 활성
주소: md_to_json
참조: 없음
...
def main():
    """메인 함수"""
    if len(sys.argv) != 2:
        print("사용법: python md_to_json.py <md_file_path>")
        sys.exit(1)
