#!/usr/bin/env python3
"""
생성 시간: 2025-08-09 10:41:00 KST
핵심 내용: Part 2 Scalability 리프 노드의 텍스트 경계를 기반으로 개별 MD 파일 추출
상세 내용:
  ...
  - process_part2_leaf_nodes (라인 101-130): Part 2 전체 처리
상태: 활성
주소: extract_part2_leaf_content
...
def process_part2_leaf_nodes():
    """Part 2 리프 노드들을 처리하여 개별 MD 파일로 저장"""
    base_dir = '/home/nadle/projects/Knowledge_Sherpa/v2/25-08-09'
    
    # 입력 파일들
    boundaries_file = os.path.join(base_dir, 'part2_scalability_leaf_nodes_with_boundaries.json')
    md_file = '/home/nadle/projects/Knowledge_Sherpa/v2/25-08-07/extracted_parts/Part_02_Part_2_Scalability.md'
    
    # 출력 폴더 생성
    output_dir = create_output_folder(base_dir, 'part2_scalability_leaf_nodes')
    
    # 데이터 로드
    nodes = load_json_data(boundaries_file)
    md_content = read_md_file(md_file)
    
    print(f"Processing Part 2 Scalability - {len(nodes)} leaf nodes...")
