...
{{
    ...
    "model_response": "실제 답변 내용"
            ...
            return {
                'success': False,
                'error': str(e),
                'has_relevant_content': None,  # 에러로 인해 판단 불가
                'elapsed_time': round(elapsed_time, 2),
                'timestamp': datetime.now().isoformat()
            }
        ...
        for i, task_result in enumerate(task_results):
            if isinstance(task_result, Exception):
                result_data = {
                    ...
                    'current_query': query,
                    'answer': '',
                    'has_relevant_content': None,  # 예외로 인해 판단 불가
                    'elapsed_time': 0.0,
                    'timestamp': datetime.now().isoformat(),
                ...
                result_data = {
                    ...
                    'has_relevant_content': task_result.get('has_relevant_content', None),
            ...
            # 개별 파일 저장은 QueryProcessor에서 담당 (중복 저장 방지)
