...
핵심 내용: Claude Code SDK를 사용해 Haiku 3.5 모델로 스크립트 파일의 핵심 내용을 추출하는 도구
상세 내용:
    - main() (라인 79-97): 메인 실행 함수, 인자 처리 및 전체 프로세스 관리
    - analyze_script_with_haiku() (라인 32-74): Claude Code SDK를 사용한 스크립트 분석 함수
    - setup_argument_parser() (라인 15-29): 명령행 인자 파싱 설정
    - 클래스 및 상수 정의 (라인 1-13): 필수 임포트 및 설정
상태: active
주소: claude_haiku_script_analyzer/v2
참조: claude_haiku_script_analyzer
"""
...
try:
    ...
    print("❌ claude-code-sdk를 찾을 수 없습니다. uv add claude-code-sdk로 설치하세요.")
    sys.exit(1)
...
def analyze_script_with_haiku(script_path: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"❌ Claude Code SDK 초기화 오류: {e}")
        return None
