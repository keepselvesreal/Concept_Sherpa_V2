...
핵심 내용: 스키마 조합 (7.4절) - allOf, anyOf를 사용한 복잡한 검증 조건 구성
상세 내용:
    - Open Library Books API 소개 (545-552행): 외부 웹 서비스 통합을 위한 API 예제
    - Watchmen 응답 예제 (553-579행): Open Library API의 실제 응답 구조
    - 기본 API 응답 스키마 (580-615행): 각 필드별 타입 정의
    - ISBN 필드 요구사항 (616-631행): isbn_10 또는 isbn_13 중 하나는 필수
    - JSON Schema 조합 개념 (632-647행): AND, OR, NOT과 같은 논리 연산자 유사
    - 스키마 분리 및 조합 (648-696행): basicBookInfoSchema, mandatoryIsbn13, mandatoryIsbn10으로 분리
    - allOf와 anyOf 사용 (686-722행): 복합 조건을 표현하는 스키마 조합
    - 표현력 향상 (723-735행): 클래스보다 높은 JSON Schema의 표현력
상태: 활성
주소: chapter7_04_schema_composition
