# 생성 시간: Thu Sep  4 16:10:11 KST 2025
# 핵심 내용: 장 추출 및 처리 서비스 (AI 서비스 통합 버전)
# 상세 내용:
#   - ChapterExtractionService (라인 18-280): 메인 장 추출 서비스 클래스
...
#   - save_chapter_content_to_folder (라인 197-265): 장별 폴더 생성 및 저장 메서드
# 상태: active
# 참조: chapter_extraction_service.py (AI 제공자 중복 제거), ai_service_v2.py 통합
...
JSON만 응답:"""
    ...
    def save_chapter_content_to_folder(self, chapter_title: str, chapter_items: List[Dict], 
        ...
        try:
            ...
            chapter_folder = Path(output_base_dir) / f"chapter_{normalized_title}"
            chapter_folder.mkdir(parents=True, exist_ok=True)
            
            # 장 내용 저장
            content_file = chapter_folder / "content.txt"
            with open(content_file, 'w', encoding='utf-8') as f:
                f.write(f"장 제목: {chapter_title}\n")
                f.write(f"생성 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("=" * 50 + "\n\n")
                f.write(chapter_content)
            
            # 목차 항목 저장
            items_file = chapter_folder / "toc_items.json"
            with open(items_file, 'w', encoding='utf-8') as f:
                json.dump(chapter_items, f, ensure_ascii=False, indent=2)
            
            # 요약 정보 저장
            summary_file = chapter_folder / "summary.json"
            summary_data = {
                "chapter_title": chapter_title,
                "normalized_title": normalized_title,
                "content_length": len(chapter_content),
                "toc_items_count": len(chapter_items),
                "created_at": datetime.now().isoformat(),
                "files": {
                    "content": str(content_file),
                    "toc_items": str(items_file),
