...
# 핵심 내용: TocService의 Solitary Unit 테스트 (격리된 단위 테스트, 외부 의존성 없음)
# 상세 내용:
#   - TestTocServiceSolitary (라인 20-135): TocService의 격리된 단위 테스트 클래스
#   - test_extract_toc_structure_with_valid_pdf (라인 25-45): PDF 목차 추출 기본 동작 테스트
#   - test_process_toc_hierarchy_with_extracted_data (라인 47-70): 목차 계층 구조 처리 테스트
#   - test_calculate_toc_page_ranges_with_processed_data (라인 72-95): 페이지 범위 계산 테스트
#   - test_extract_complete_toc_full_workflow (라인 97-135): 완전한 목차 추출 워크플로우 테스트
# 상태: active
# 주소: tests/unit/solitary/test_toc_service_solitary
# 참조: TestTocExtractorSocial에서 추출한 실제 데이터 활용
...
# TocService import를 위한 경로 설정
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "src"))

# TocService와 기존 유틸리티 import
from services.toc_service import TocService
from utils.config_manager import ConfigManager
from utils.logger import LoggerFactory

@pytest.mark.solitary_unit
class TestTocServiceSolitary:
    """
    TocService의 Solitary Unit 테스트
    - 외부 의존성 없는 격리된 테스트
    - 기존 toc_extractor 로직과 동일한 동작 검증
    - 새 아키텍처 패턴 적용 (config_manager, logger 의존성 주입)
    """
    
    def test_extract_toc_structure_with_valid_pdf(self, real_pdf_path):
        """
        ...
        """
        # Given: TocService 인스턴스 생성 (config_manager와 logger 주입)
        ...
        pytest.skip("TocService 구현 후 테스트 실행")
    
    def test_process_toc_hierarchy_with_extracted_data(self, real_pdf_path):
        """
        ...
        """
        # Given: 추출된 기본 목차 데이터
        # toc_service = TocService(MockConfigManager(), MockLogger())
        # raw_toc_items = toc_service.extract_toc_structure(real_pdf_path)
    ...
    def test_calculate_toc_page_ranges_with_processed_data(self, real_pdf_path):
        """
    ...
    def test_extract_complete_toc_full_workflow(self, real_pdf_path):
        """
        ...
        # first_complete_item = toc_structure[0]
        # for field in complete_fields:
        #     assert field in first_complete_item, f"완전한 목차 항목에 {field} 필드가 있어야 함"
