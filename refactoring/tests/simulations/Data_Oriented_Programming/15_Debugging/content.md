# 15 Debugging
(생성일시: 2025-09-10T21:04:27.478219)
...
Yes.
Theo
Well, that day may have come. I think I have discovered an interesting connec-
...
Debugging
Dave
OK. For the number and the string, I can simply copy their values to the clip-
...
Dave
Cool! I’ll fix the code in hasWordStartingWith that captures the string argu-
ments. Here you go.
function hasWordStartingWith(sentence, prefix) {
...
Why?
Theo
Because the REPL environment gives you a much quicker feedback than trig-
...
Listing 15.12
An improved version of book search
...
var query = "Habit";
Listing 15.13
Testing searchBooksByTitle again
Listing 15.14
A unit test with captured data
...
as the unit tests.
Dave
There are so many files! How do we make sure a file doesn’t override an exist-
ing file?
Theo
By following a simple file-naming convention. A name for a file that stores cap-
...
Theo
In some languages it’s part of the language, but in other languages like Java-
...
Listing 15.15
Computing the file path for storing captured data
Listing 15.16
Dumping data in JSON format
Listing 15.17
Capturing the context into a file
The root folder 
for captured data
...
Console output when triggering the search endpoint
Listing 15.19
The content of the JSON file that captured the context
Listing 15.20
Dumping data in JSON format with indentation
...
Listing 15.21
The captured context with indentation in the JSON file
...
Dave
In DOP, we never mutate data. Therefore, my question is no longer a ques-
...
Listing 15.23
A unit test that reads captured data from a file
...
Theo
OK. Let’s fix the code of the improved search so that it works with the two que-
...
Dave
The first query doesn’t work because the code leaves the strings in their origi-
...
No.
Theo
\b matches a position that is called a word boundary. It allows us to perform pre-
fix matching.
Dave
...
It also matches.
Dave
Excellent. This is exactly what I need! Let me fix the code of hasWordStart-
...
Listing 15.24
A revised version of hasWordStartingWith
...
// →[true, true, true, true]
Listing 15.25
A unit test for several queries that should return a book
Listing 15.26
A unit test for several queries that should return no book
...
Dave
Now, am I allowed to trigger the search endpoint with 7 Habit in order to con-
...
Why not?
Dave
The function context might be exactly the same, but the behavior might be dif-
...
banks.
 NOTE
See https://www.datomic.com for more information on the Datomic transac-
tional database.
Dave
But most databases don’t provide such a guarantee!
Theo
Right, but in practice, when we’re debugging an issue in our local environ-
...
Dave
I see. Because we are at the Museum of Science, would you allow me an anal-
ogy with science?
Theo
Of course!
Dave
...
A book?
Theo
Yes. DOP has been a meaningful journey for me. I have learned some interest-
