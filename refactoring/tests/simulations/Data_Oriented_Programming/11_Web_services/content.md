# 11 Web services
(생성일시: 2025-09-10T21:04:27.475276)
...
Cool!
Theo
Nancy wants to enrich the book search results. Instead of fetching book infor-
...
languages.
Joe
What the idiom says is that, inside our program, the inner components of a pro-
...
Joe
In DOP, as we have seen so far, the inner components of a program communi-
...
Joe
From a design perspective, it’s important because it means that the inner com-
ponents of a program are loosely coupled.
Theo
What do you mean by loosely coupled?
Joe
I mean that components need no knowledge about the internals of other com-
...
and one for desktop.
Joe
What about situations where the client wants to display different pieces of infor-
...
return
_.set(relevantInfo, "isbn", isbn);
}
}
 NOTE
The JavaScript snippets of this chapter are written as if JavaScript were deal-
...
Joe
True, and classes don’t bring any added value because we need to pass data for-
...
Joe
No! As you might remember from one of our previous sessions, Lodash pro-
...
"weight": {"type": "string"},
Listing 11.10
A map with book information from the database
Listing 11.11
A map with book information from the Open Library Books API
Listing 11.12
Merging two maps
Listing 11.13
JSON schema for search books response
...
Theo
I think I’ll start with the implementation of the book retrieval from the data-
...
Send the response to the client.
Listing 11.14
Retrieving books whose title matches a query
...
"publish_date", "weight",
Listing 11.15
Retrieving book information from Open Library for several books
...
Theo
Yeah. I have two arrays in my hands: one with book information from the data-
...
_.keyBy(books, "isbn");
Listing 11.16
Transforming an array into a map with _.keyBy
...
The result of keyBy
Listing 11.18
A generic function for joining arrays
...
Listing 11.20
Search books and enriched book information
Listing 11.21
Schema for the extended search endpoint (Open Books API part)
...
"publish_date", "weight",
Listing 11.22
Extended search endpoint (Open Books API part)
...
Listing 11.23
Extended search endpoint (database part)
...
Listing 11.24
Schema for the implementation of the extended search endpoint
Listing 11.25
Schema for the extended search endpoint (combines the pieces)
...
TIP
Classes are much less complex when we use them as a means to aggregate state-
...
Description
keyBy(coll, f)
Creates a map composed of keys generated from the results of running each ele-
