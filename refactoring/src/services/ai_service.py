...
class AIService:
    ...
    def _get_provider(self) -> AIProvider:
        """설정에 따른 AI 제공자 선택"""
        # 단계별 AI 설정 (예: workspace_preparation.ai, content_processing.ai)
        stage_ai_config = self.config_manager.get(f"{self.stage_name}.ai_analysis", config_type="pipeline")
        
        # 전역 AI 설정 fallback
        if not stage_ai_config:
            stage_ai_config = self.config_manager.get("ai", config_type="ai")
