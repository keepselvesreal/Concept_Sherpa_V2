...
def load_previous_context(session_folder: Path, current_query_number: int, logger: logging.Logger) -> Optional[Dict]:
    """직전 질의와 응답 로드 (캐시 기반 단순 로직)"""
    try:
        # 직전 대화는 현재 query_number - 1
        previous_query_number = current_query_number - 1
        
        if previous_query_number < 1:
            logger.error("첫 번째 질의이므로 직전 대화가 존재하지 않습니다")
            return None
        
        # 직전 대화 파일 경로
        previous_file = session_folder / f"collective_{previous_query_number}_answer.json"
        
        if not previous_file.exists():
            logger.error(f"직전 대화 데이터를 찾을 수 없습니다: {previous_file}")
            return None
        
        # 직전 대화 파일 로드
        with open(previous_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            logger.info(f"직전 대화 로드: query_number={previous_query_number}")
            return {
                'query_number': data.get('query_number'),
                'query': data.get('query'),
                'response': data.get('model_response')
            }
            
    except Exception as e:
        logger.error(f"직전 대화 내용 로드 중 오류: {e}")
        return None


async def analyze_relevance(previous_query: str, previous_response: str, current_query: str, 
                           config: Dict, logger: logging.Logger) -> Dict:
    """AI를 통한 연관성 분석 및 보충 질의 생성"""
    start_time = time.time()
    
    try:
        # 연관성 분석 프롬프트
        prompt = f"""다음 정보를 바탕으로 직전 대화와 현재 질의의 연관성을 분석하고, 사용자의 이해 부족 부분을 파악해주세요.
...
{{
    ...
    "generated_query": "사용자 이해 부족 부분을 보충하기 위한 질의 (연관성이 없으면 '관련 없음')"
}}

연관성 판단 기준:
1. 주제나 개념의 연속성
2. 사용자의 추가 질문이나 심화 학습 의도
3. 이전 답변에서 언급된 내용에 대한 후속 질문
4. 관련 분야나 응용 사례에 대한 궁금증
