#!/usr/bin/env python3
"""
TOC v2 기반 폴더 구조 생성기 (수정판)
- 1.1 -> 1-1, A.1 -> A-1 형태로 폴더명 변경
- Appendices의 단일 문자 폴더 제거
- metadata.json 파일 생성하지 않음
"""
...
class TOCFolderCreatorV2Fixed:
    ...
    def parse_toc_v2(self) -> None:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Appendices의 단일 문자 항목 스킵 (A, B, C, D... 등)
            if self._should_skip_single_letter(title, level):
                continue
    ...
    def _should_skip_single_letter(self, title: str, level: int) -> bool:
        """Appendices 하위의 단일 문자 항목 스킵 여부"""
        # node2 레벨에서 단순히 한 글자인 경우 (A, B, C, D 등)
        if level == 2 and re.match(r'^[A-W]$', title.strip()):
            return True
        return False
    ...
    def create_folder_structure(self) -> None:
        ...
        # 재귀적으로 internal node 폴더들 생성 (leaf node는 폴더로 만들지 않음)
    ...
    def _sanitize_folder_name(self, title: str) -> str:
        ...
        # 점(.)을 하이픈(-)으로 변경 (1.1 -> 1-1, A.1 -> A-1)
        clean_name = re.sub(r'\.', '-', title)
        
        # 기타 문제가 되는 문자들 제거/변환
        clean_name = re.sub(r'[<>:"/\\|?*]', '', clean_name)
        clean_name = re.sub(r'[—–]+', '_', clean_name)  # 다양한 대시를 언더스코어로 (단, 하이픈 제외)
        clean_name = re.sub(r'\s+', '_', clean_name)      # 공백을 언더스코어로
        clean_name = re.sub(r'[^\w\s_-]', '', clean_name) # 특수문자 제거 (하이픈은 유지)
    ...
    def _create_structure_summary(self) -> None:
        ...
        # JSON 요약
        summary_file = self.output_base_dir / "folder_structure_summary_fixed.json"
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)
        
        # 읽기 쉬운 텍스트 요약
        text_summary_file = self.output_base_dir / "folder_structure_summary_fixed.md"
        with open(text_summary_file, 'w', encoding='utf-8') as f:
            f.write("# 폴더 구조 요약 (수정판)\n\n")
            ...
            f.write("## 수정사항\n")
            f.write("- 폴더명: 1.1 -> 1-1, A.1 -> A-1\n")
            f.write("- Appendices의 단일 문자 폴더(A, B, C 등) 제거\n")
            f.write("- metadata.json 파일 생성 안함\n\n")
    ...
    def _generate_folder_tree_text(self, node: TOCNodeV2, indent: str) -> str:
        ...
        if node.level == 0:
            ...
            if not node.is_leaf:
                ...
                result += f"{indent}📄 {node.title} ({node.node_type}) [LEAF - 파일 생성 대상]\n"
                return result
...
def main():
    ...
    print("🚀 TOC v2 기반 폴더 구조 생성 시작 (수정판)")
    print(f"📄 TOC 파일: {toc_file}")
    print(f"📁 출력 디렉토리: {output_dir}")
    print("수정사항: 1.1->1-1, A.1->A-1, 단일문자폴더제거, metadata없음")
    
    creator = TOCFolderCreatorV2Fixed(toc_file, output_dir)
