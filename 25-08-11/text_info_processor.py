"""
생성 시간: 2025-08-11 12:02:34 KST
핵심 내용: Claude SDK를 이용한 텍스트 파일 정보 분석 및 구조화된 정보 파일 생성 (병렬처리)
상세 내용:
    ...
    - 4개 분석 작업을 병렬로 동시 실행
상태: 활성
주소: text_info_processor
참조: parallel_leaf_extractor.py (Claude SDK 사용법 참고)
"""
...
async def process_text_file(source_path):
    ...
    print(f"소스 파일 읽기 완료: {source_path}")
...
async def main():
    source_file = "/home/nadle/projects/Knowledge_Sherpa/v2/25-08-11/extracted_sections/7.1_Data_validation_in_DOP.md"
    await process_text_file(source_file)
