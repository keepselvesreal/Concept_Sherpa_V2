...
핵심 내용: 상위-하위 구성 단위 간격에서 Introduction 항목을 생성하고 기존 TOC에 삽입하여 _v2 파일 생성
상세 내용:
    - load_gaps_data(라인 15-25): 간격 데이터 JSON 파일 로드
    - generate_introduction_title(라인 27-65): 하위 구성 단위 표기 체계 기반 Introduction 제목 생성
    - create_introduction_items(라인 67-95): Introduction 항목들 생성
    - insert_introductions_to_toc(라인 97-125): 기존 TOC에 Introduction 항목들 삽입
    - save_updated_toc(라인 127-140): 수정된 TOC를 _v2 파일로 저장
    - main(라인 142-165): 메인 실행 함수
상태: 활성
주소: toc_introduction_generator
참고: toc_content_gaps.json과 core_toc_with_page_ranges.json을 기반으로 작성
"""
...
def generate_introduction_title(parent_title: str, child_title: str) -> str:
    """하위 구성 단위의 표기 체계를 기반으로 Introduction 제목을 생성합니다."""
    ...
    # Introduction 번호 생성 로직
    if child_number:
        if re.match(r'^[ABC]$', child_number):  # A, B, C
            intro_number = f"{child_number}.0"
        elif re.match(r'^[ABC]\.\d+$', child_number):  # A.1, B.2 등
            intro_number = f"{child_number}.0"
        elif re.match(r'^\d+$', child_number):  # 1, 2, 3 등
            intro_number = f"{child_number}.0"
        elif re.match(r'^\d+\.\d+$', child_number):  # 1.1, 2.3 등
            intro_number = f"{child_number}.0"
        elif re.match(r'^\d+\.\d+\.\d+$', child_number):  # 1.1.1, 2.3.4 등
...
def main():
    ...
    output_file = "/home/nadle/projects/Knowledge_Sherpa/v2/25-08-07/core_toc_with_page_ranges_v2.json"
