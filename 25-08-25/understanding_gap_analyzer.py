...
class UnderstandingGapAnalyzer:
    ...
    async def analyze_understanding_gap(self, previous_query: str, previous_answer: str, current_query: str) -> Dict[str, Any]:
        ...
        try:
            analysis_prompt = """다음 대화를 분석하여 현재 질의가 이전 질의/답변과 관련이 있는지 먼저 판단하고, 관련이 있다면 사용자가 이해하지 못한 부분을 파악하여 보완 질의를 생성해주세요.
...
{{
    "is_related": true/false (현재 질의가 이전 질의/답변과 관련이 있는지 여부),
    "analysis": "관련이 있다면 사용자가 이해하지 못한 부분에 대한 상세한 분석, 관련이 없다면 '해당 사항 없음'",
    "generated_query": "관련이 있다면 이해 부족 부분을 보완하기 위한 구체적인 질의문, 관련이 없다면 '해당 사항 없음'"
}}""".format(
            ...
            async with ClaudeSDKClient() as client:
                ...
                try:
                    ...
                    return {
                        'success': False,
                        'error': f'JSON 파싱 실패: {response_text}',
                        'is_related': None,
                        'analysis': '',
                        'generated_query': ''
                    }
...
중요 지침: 
- 참고 문서에 질의와 관련된 내용이 있으면 그 내용을 바탕으로 상세히 답변하세요.
- 참고 문서에 질의와 관련된 내용이 없다면 "질의 관련 내용 없음"이라고만 답변하세요."""
