...
## 핵심 내용
이 문서는 객체 지향 프로그래밍(OOP)에서 클래스 상속을 사용하여 코드를 재사용하는 방법과 한계를 설명한다. 새로운 요구사항이 추가될 때 기존 클래스 계층 구조가 복잡해지고, 다이아몬드 상속 문제와 같은 설계 문제가 발생할 수 있음을 보여준다. 따라서 클래스 상속보다는 컴포지션(composition)을 사용하는 것이 더 나은 설계 방법일 수 있음을 시사한다.

## 상세 핵심 내용
OOP에서 클래스 상속은 코드 재사용에 유용하지만, 시스템 요구사항이 변경되면 기존 클래스 계층 구조에 새로운 클래스를 추가해야 하는 어려움이 발생할 수 있다. VIP 멤버와 Super 멤버 기능을 추가하는 과정에서 Librarian 클래스의 코드를 재사용하기 위해 VIPMember 클래스를 설계해야 하지만, 다중 상속으로 인해 다이아몬드 상속 문제가 발생한다. 이러한 복잡성을 피하기 위해 User 클래스를 인터페이스로 바꾸고 컴포지션(composition)을 사용하는 것이 좋지만, 데드라인 압박으로 인해 실패한다. 따라서 클래스 상속보다는 컴포지션을 사용하는 것이 더 유연하고 유지보수가 용이한 시스템을 구축하는 데 도움이 될 수 있다.

## 상세 정보
*   OOP에서 클래스 상속은 코드 재사용을 위한 일반적인 방법이다.
*   초기 요구사항이 명확할 때는 클래스 계층 구조를 설계하기 용이하다.
*   새로운 요구사항 추가는 기존 클래스 계층 구조에 복잡성을 더할 수 있다.
*   VIP 멤버는 도서관에 책을 추가할 수 있는 권한을 가진다.
*   VIPMember 클래스를 설계할 때 Librarian 클래스의 코드를 재사용하는 것이 문제된다.
*   다중 상속은 다이아몬드 상속 문제와 같은 설계 문제를 야기할 수 있다.
*   Super 멤버는 다른 멤버에게 대여된 책 목록을 볼 수 있는 권한을 가진다.
*   컴포지션은 클래스 상속보다 더 유연하고 유지보수가 용이한 설계 방법일 수 있다.
*   데드라인 압박은 설계 결정을 어렵게 만들고 시스템 복잡성을 증가시킬 수 있다.
*   프로젝트는 Super 멤버 기능 추가 실패로 데드라인을 맞추지 못했지만, 해당 기능은 나중에 추가하기로 결정되었다.
...
*   컴포지션 (Composition)
*   객체 지향 프로그래밍 (OOP)
*   요구사항 변경 (Changing Requirements)
*   코드 재사용 (Code Reuse)
*   시스템 설계 (System Design)

## 부차 화제
*   VIP 멤버 (VIP Members)
*   Super 멤버 (Super Members)
