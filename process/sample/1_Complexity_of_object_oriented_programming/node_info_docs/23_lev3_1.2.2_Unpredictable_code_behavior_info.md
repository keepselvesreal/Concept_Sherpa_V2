...
## 핵심 내용
객체 지향 프로그래밍(OOP)에서 데이터의 가변성은 멀티스레드 환경에서 예측 불가능한 코드 동작을 초래할 수 있다. 데이터 불변성을 강조하는 데이터 중심 프로그래밍(DOP)은 이러한 문제를 해결하고 안정적인 개발 환경을 제공한다.
...
## 상세 핵심 내용
OOP에서 객체 멤버의 가변성은 코드의 예측 불가능성을 야기하며, 이는 특히 멀티스레드 환경에서 두드러진다. 두 번의 console.log 호출 사이에 객체의 상태가 변경될 수 있기 때문이다. 이러한 문제를 해결하기 위해 뮤텍스와 같은 동시성 안전 메커니즘을 사용할 수 있지만, 성능 저하 및 데드락의 위험이 따른다. DOP는 모든 데이터를 불변 값으로 취급하여 이러한 문제를 해결하고 개발자에게 안정적인 환경을 제공하며, 데이터 불변성은 DOP 개발자에게 정신적인 평화를 가져다준다.
