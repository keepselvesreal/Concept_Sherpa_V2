"""
생성 시간: 2025-08-21 15:50:20 KST
핵심 내용: Claude Code SDK를 사용하여 YouTube 대본을 적절한 단위로 합치고 필러 워드를 제거하며 구조화하는 스크립트
상세 내용:
    - parse_transcript(file_path): 마크다운 파일에서 대본 데이터 파싱
    - improve_transcript_with_claude(transcript_entries): Claude Code SDK로 대본 개선
    ...
    - main(): 메인 실행 함수
상태: 활성
주소: transcript_improver_v2
참조: transcript_improver, claude_response_generator_with_save
"""
...
async def improve_transcript_with_claude(transcript_entries: List[Dict]) -> str:
    """
    Claude Code SDK를 사용하여 대본을 개선합니다.
    ...
    try:
        ...
        # 대본 텍스트 준비 (처음 100개 항목으로 제한)
        transcript_text = ""
        limited_entries = transcript_entries[:100]
        for entry in limited_entries:
            transcript_text += f"Line {entry['line_number']}: {entry['timestamp']} {entry['text']}\n"
        
        if len(transcript_entries) > 100:
            print(f"   📝 처리량 제한: {len(transcript_entries)}개 중 처음 100개 항목만 처리")
...
async def main():
    ...
    # Claude Code SDK로 대본 구조화
    print("🤖 Claude Code SDK로 대본 구조화 중...")
    print("   - 필러 워드 제거")
    print("   - 의미 단위 문장 합치기")
    print("   - 5-7개 섹션으로 구분")
    print("   - 라인 번호 유지")
    
    improved_content = await improve_transcript_with_claude(transcript_entries)
