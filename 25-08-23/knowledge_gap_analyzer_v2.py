...
- 핵심 내용: 이전 질문-답변과 현재 질문을 분석해 사용자 미이해 부분을 도출하고 보완 질의문을 생성하며 JSON 파일로 저장하는 시스템
- 상세 내용:
    - KnowledgeGapAnalyzerV2 클래스 (라인 31-181): 사용자 이해도 분석 및 질의문 생성을 담당하는 메인 클래스 (파일 저장 기능 추가)
    ...
    - save_questions_to_file 함수 (라인 143-155): 생성된 질의문을 JSON 파일로 저장하는 함수
    - process_gap_analysis 함수 (라인 157-181): 전체 갭 분석 프로세스를 관리하고 질의문 파일을 저장하는 함수
    - test_knowledge_gap_analyzer_v2 함수 (라인 183-249): 시스템 테스트 함수 (파일 저장 기능 포함)
    - 갭 분석 로직 (라인 56-87): 이전 응답과 현재 질문의 차이점을 Claude로 분석
    - 질의문 저장 로직 (라인 172-181): 생성된 질의문을 구조화된 JSON 파일로 저장
- 상태: active
- 참조: knowledge_gap_analyzer.py를 기반으로 파일 저장 기능 추가
"""
...
class KnowledgeGapAnalyzerV2:
    """사용자 이해도 분석 및 질의문 생성을 담당하는 메인 클래스 (파일 저장 기능 추가)"""
...
{{
  "followup_questions": [
    ...
    def save_questions_to_file(self, questions_data: Dict[str, Any], reference_documents: List[str]) -> str:
        """생성된 질의문을 JSON 파일로 저장하는 함수"""
        questions_file = f"/home/nadle/projects/Concept_Sherpa_V2/25-08-23/followup_questions_{self.session_id}.json"
    ...
    async def process_gap_analysis(self, previous_qa: Dict[str, str], 
        ...
        if questions_result['status'] == 'success':
            questions_file = self.save_questions_to_file(questions_result, reference_documents)
...
async def test_knowledge_gap_analyzer_v2():
    """시스템 테스트 함수 (파일 저장 기능 포함)"""
    print("=== Knowledge Gap Analyzer V2 (파일 저장) 테스트 시작 ===\n")
...
- 여기에 투자하면 출력 품질이 극적으로 향상됨'''
    ...
    analyzer = KnowledgeGapAnalyzerV2()
    ...
    try:
        ...
        if result['questions_file']:
            print(f"📁 질의문 파일 저장: {os.path.basename(result['questions_file'])}")
        ...
        print(f"📋 전체 결과 저장: {os.path.basename(results_file)}")
...
if __name__ == "__main__":
    asyncio.run(test_knowledge_gap_analyzer_v2())