"""
# 목차
- 생성 시간: 2025년 8월 23일 13:35:26 KST
- 핵심 내용: Claude SDK를 사용해 여러 참고 문서에 대해 병렬로 질문 답변을 생성하며, 각 답변이 완료되는 즉시 개별 파일로 저장하는 시스템
- 상세 내용:
    - DocumentBasedQASystemV2 클래스 (라인 31-177): 실시간 개별 저장 기능이 추가된 문서 기반 질의응답 시스템
    - get_response_for_document 함수 (라인 36-86): 개별 문서에 대해 Claude로부터 답변을 받는 비동기 함수 (기존과 동일)
    - save_individual_result 함수 (라인 88-100): 각 답변을 개별 JSON 파일로 즉시 저장하는 함수
    - process_documents_realtime 함수 (라인 102-160): asyncio.as_completed를 사용한 실시간 처리 함수
    - main 함수 (라인 162-177): 새로운 실시간 처리 방식을 사용하는 메인 함수
    - test_document_qa_system_v2 함수 (라인 179-242): 실시간 개별 저장 테스트 함수
    - 실시간 출력 (라인 143-158): 각 문서 답변이 완료되는 즉시 출력 및 개별 파일 저장
    - 최종 요약 저장 (라인 225-235): 모든 처리 완료 후 전체 요약 정보를 별도 파일로 저장
- 상태: active
- 참조: document_based_qa_system.py를 기반으로 실시간 개별 저장 기능 추가
"""
...
class DocumentBasedQASystemV2:
    """실시간 개별 저장 기능이 추가된 문서 기반 질의응답 시스템"""
...
async def test_document_qa_system_v2():
    """실시간 개별 저장 문서 기반 질의응답 시스템 테스트 함수"""
    print("=== Claude SDK 문서 기반 QA 시스템 V2 (실시간 저장) 테스트 시작 ===\n")
    ...
    qa_system = DocumentBasedQASystemV2()
...
if __name__ == "__main__":
    asyncio.run(test_document_qa_system_v2())