"""
# 목차
- 생성 시간: 2025년 8월 23일 12:47:30 KST
- 핵심 내용: Claude SDK를 사용한 다중 에이전트 동시 실행 시스템 (Haiku 3 모델, 올바른 옵션 설정)
- 상세 내용:
    - MultiAgentClaudeClient 클래스 (라인 21-95): 여러 Claude 에이전트를 관리하고 동시 실행하는 메인 클래스
    - get_response_with_metadata 함수 (라인 28-50): 개별 에이전트의 응답을 메타데이터와 함께 수집하는 비동기 함수  
    - run_agents_concurrently 함수 (라인 52-79): 여러 에이전트를 동시에 실행하고 결과를 수집하는 함수
    - main 함수 (라인 81-95): 테스트 시나리오를 실행하는 메인 함수
    - test_multi_agent_system 함수 (라인 97-151): 실제 테스트를 수행하는 함수
    - 에러 핸들링 (라인 124-134): CLI 설치 오류 및 프로세스 오류 처리
    - 결과 출력 (라인 135-151): 각 에이전트의 응답과 메타데이터를 구조화하여 출력
    - Haiku 3 모델 설정 수정 (라인 31-35): ClaudeSDKClient 생성 시 옵션 설정
- 상태: 활성
- 주소: multi_agent_claude_tester_haiku3_v2
- 참조: multi_agent_claude_tester_haiku3를 수정하여 올바른 옵션 설정 방법 적용
"""
...
class MultiAgentClaudeClient:
    ...
    def __init__(self):
        self.results = {}
        # Haiku 3 모델 옵션 설정 - ClaudeSDKClient 생성 시 사용
...
async def test_multi_agent_system():
    ...
    print("=== Claude SDK 다중 에이전트 테스트 시작 (Haiku 3 모델 v2) ===\n")
    ...
    try:
        ...
        # 결과를 JSON 파일로 저장
        results_file = f"/home/nadle/projects/Concept_Sherpa_V2/25-08-23/multi_agent_results_haiku3_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
