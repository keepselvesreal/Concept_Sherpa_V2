"""
# 목차
- 생성 시간: 2025년 8월 23일 12:48:40 KST
- 핵심 내용: Claude SDK를 사용한 다중 에이전트 동시 실행 시스템 (Haiku 3 모델, max_tokens 제한 해결)
- 상세 내용:
    - MultiAgentClaudeClient 클래스 (라인 21-96): 여러 Claude 에이전트를 관리하고 동시 실행하는 메인 클래스
    - get_response_with_metadata 함수 (라인 28-51): 개별 에이전트의 응답을 메타데이터와 함께 수집하는 비동기 함수  
    - run_agents_concurrently 함수 (라인 53-80): 여러 에이전트를 동시에 실행하고 결과를 수집하는 함수
    - main 함수 (라인 82-96): 테스트 시나리오를 실행하는 메인 함수
    - test_multi_agent_system 함수 (라인 98-152): 실제 테스트를 수행하는 함수
    - 에러 핸들링 (라인 125-135): CLI 설치 오류 및 프로세스 오류 처리
    - 결과 출력 (라인 136-152): 각 에이전트의 응답과 메타데이터를 구조화하여 출력
    - Haiku 3 모델 설정 수정 (라인 31-36): max_tokens를 4096으로 제한하여 Haiku 3 모델 제한사항 해결
- 상태: 활성
- 주소: multi_agent_claude_tester_haiku3_v3
- 참조: multi_agent_claude_tester_haiku3_v2를 수정하여 max_tokens 제한 문제 해결
"""
...
class MultiAgentClaudeClient:
    ...
    def __init__(self):
        self.results = {}
        # Haiku 3 모델 옵션 설정 - max_tokens를 4096으로 제한
        self.options = ClaudeCodeOptions(
            model="claude-3-haiku-20240307",  # Haiku 3 모델 지정
            system_prompt="You are a helpful coding assistant",
            max_turns=5,
            max_tokens=4000  # Haiku 3 모델의 최대 출력 토큰 제한 (4096보다 작게 설정)
        )
    ...
    async def run_agents_concurrently(self, agent_configs: List[Dict[str, str]]) -> Dict[str, Any]:
        """여러 에이전트를 동시에 실행하고 결과를 수집 (Haiku 3 모델 사용)"""
        tasks = []
    ...
    async def main(self):
        """메인 실행 함수 (Haiku 3 모델 사용)"""
        agent_configs = [
            {
                'name': 'code_reviewer_haiku3',
                'prompt': 'Python 코드의 일반적인 품질 체크포인트를 3가지만 간단하게 알려주세요'
            },
            {
                'name': 'test_writer_haiku3', 
                'prompt': 'Python 함수를 테스트할 때 고려해야 할 핵심 사항 3가지를 간단하게 알려주세요'
            },
            {
                'name': 'documentation_writer_haiku3',
...
async def test_multi_agent_system():
    """다중 에이전트 시스템 테스트 함수 (Haiku 3 모델 사용)"""
    print("=== Claude SDK 다중 에이전트 테스트 시작 (Haiku 3 모델 v3) ===\n")
    
    client = MultiAgentClaudeClient()
    
    try:
        # 1. 단일 에이전트 테스트
        print("1. 단일 에이전트 테스트 (Haiku 3)...")
        result = await client.get_response_with_metadata(
            "안녕하세요! 간단한 인사말을 해주세요.",
            "greeting_agent_haiku3"
        )
        ...
        # 2. 다중 에이전트 동시 실행 테스트
        print("2. 다중 에이전트 동시 실행 테스트 (Haiku 3)...")
        ...
        # 결과를 JSON 파일로 저장
        results_file = f"/home/nadle/projects/Concept_Sherpa_V2/25-08-23/multi_agent_results_haiku3_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
