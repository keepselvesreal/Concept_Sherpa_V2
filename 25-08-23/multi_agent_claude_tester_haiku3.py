"""
# 목차
- 생성 시간: 2025년 8월 23일 12:44:20 KST
- 핵심 내용: Claude SDK를 사용한 다중 에이전트 동시 실행 시스템 (Haiku 3 모델 사용)
- 상세 내용:
    - MultiAgentClaudeClient 클래스 (라인 21-102): 여러 Claude 에이전트를 관리하고 동시 실행하는 메인 클래스
    - get_response_with_metadata 함수 (라인 28-57): 개별 에이전트의 응답을 메타데이터와 함께 수집하는 비동기 함수
    - run_agents_concurrently 함수 (라인 59-86): 여러 에이전트를 동시에 실행하고 결과를 수집하는 함수
    - main 함수 (라인 88-102): 테스트 시나리오를 실행하는 메인 함수
    - test_multi_agent_system 함수 (라인 104-158): 실제 테스트를 수행하는 함수
    - 에러 핸들링 (라인 131-141): CLI 설치 오류 및 프로세스 오류 처리
    - 결과 출력 (라인 142-158): 각 에이전트의 응답과 메타데이터를 구조화하여 출력
    - Haiku 3 모델 설정 (라인 36-40): claude-3-haiku-20240307 모델 설정
- 상태: 활성
- 주소: multi_agent_claude_tester_haiku3
- 참조: multi_agent_claude_tester 파일을 기반으로 Haiku 3 모델로 수정
"""
...
class MultiAgentClaudeClient:
    ...
    async def get_response_with_metadata(self, client: ClaudeSDKClient, prompt: str, agent_name: str) -> Dict[str, Any]:
        ...
        try:
            # Haiku 3 모델로 쿼리 실행
            await client.query(prompt, options=self.options)
            text_parts = []
            total_cost = 0.0
...
async def test_multi_agent_system():
    ...
    print("=== Claude SDK 다중 에이전트 테스트 시작 (Haiku 3 모델) ===\n")
    ...
    try:
        ...
        # 결과를 JSON 파일로 저장
        results_file = f"/home/nadle/projects/Concept_Sherpa_V2/25-08-23/multi_agent_results_haiku3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
