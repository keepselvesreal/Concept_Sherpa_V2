...
핵심 내용: JSON 구조 파싱 및 노드 관계 분석 시스템 - 간소화된 버전
상세 내용:
    - Node 클래스 (라인 26-): 노드 정보와 관계 캡슐화
    - NodeStructureAnalyzer 클래스 (라인 59-): JSON 구조 분석 및 트리 구축
    - load_json_structure() (라인 68-): JSON 파일 로드 및 구조 검증
    - build_node_tree() (라인 94-): JSON 데이터를 Node 트리로 변환
    - get_leaf_nodes() (라인 159-): 리프 노드 목록 반환
    - get_nodes_by_level() (라인 163-): 레벨별 노드 분류
    - get_processing_order() (라인 172-): 하위→상위 처리 순서 생성
    - 불필요한 텍스트 파일 매핑 기능 제거
상태: 활성
주소: node_structure_analyzer_v2
참조: node_structure_analyzer.py (원본 파일)
"""
