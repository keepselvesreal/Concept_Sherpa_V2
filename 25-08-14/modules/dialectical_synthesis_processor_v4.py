"""
생성 시간: 2025-08-14 11:28:00 KST
핵심 내용: 정반합 방법론 V4 - 새로운 파일명 형식 적용
상세 내용:
    ...
    - 파일명 형식: {id:02d}_lev{level}_{title}_info.md (새로운 형식)
상태: 활성
주소: dialectical_synthesis_processor_v4
참조: dialectical_synthesis_processor_v3.py (이전 버전)
"""
...
from node_structure_analyzer_v2 import Node
...
class DataLoader:
    ...
    def _get_node_file_path(self, node: Node) -> Path:
        ...
        return self.base_dir / f"{node.id:02d}_lev{node.level}_{safe_title}_info.md"
...
class DialecticalSynthesisProcessor:
    """정반합 방법론 메인 처리 클래스 V4 - 새로운 파일명 형식 적용"""
    ...
    def __init__(self, output_dir: str):
        ...
        # 공통 모듈 초기화
        self.logger = ProcessLogger("dialectical_synthesis_v4", self.output_dir)
...
async def main():
    """테스트 실행"""
    from node_structure_analyzer_v2 import NodeStructureAnalyzer
    ...
    print("=" * 60)
    print("정반합 방법론 시스템 V4 - 새로운 파일명 형식 적용")
    print("=" * 60)
