# 생성 시간: 2025-08-31 21:30:00 KST
# 핵심 내용: 범용적 목차 구조 분석 및 장별 내용 추출 모듈
# 상세 내용:
...
#   - analyze_toc_structure_with_ai (라인 195): AI를 통한 범용적 목차 구조 분석
#   - find_chapter_items (라인 275): 각 장에 속하는 목차 항목들을 찾는 함수
#   - save_chapter_content_to_folder (라인 325): 장별 폴더에 PDF 내용과 정규화된 목차 저장
#   - extract_chapters_programmatically (라인 365): 프로그래밍적 목차 및 내용 추출 (폴더 생성 포함)
#   - process_chapters (라인 465): 전체 처리 워크플로우 (PDF 내용 추출 포함)
#   - main (라인 535): 메인 실행 함수
# 상태: active
# 참조: extract_chapters_v3
...
async def analyze_toc_structure_with_ai(toc_json_path: str, ai_provider: AIProvider, logger) -> Dict[str, Any]:
    """AI를 통한 범용적 목차 구조 분석 - 실제 장(Chapter) 단위 식별"""
    try:
        # 목차 파일 읽기
        ...
        # 향상된 AI 프롬프트 - 동적 구조 분석
        toc_json_str = json.dumps(toc_structure, ensure_ascii=False, indent=2)
        
        prompt = f"""다음은 책의 목차 구조 데이터입니다. 이 데이터를 분석하여 실제 "장(Chapter)" 단위를 식별해주세요.
...
- level: 계층 레벨 (0이 최상위)
- page: 시작 페이지
- parent_id: 부모 항목 ID (null이면 최상위)
- children_ids: 하위 항목들의 ID 리스트

목차 데이터:
{toc_json_str}

분석 작업:
1. 목차 구조를 분석하여 다음을 판단하세요:
   - 어떤 레벨의 항목들이 실제 "장(Chapter)"에 해당하는가?
   - Part, Section, Appendix 등은 제외하고 실제 내용을 담은 Chapter만 선택
   - 숫자로 된 장 번호(1, 2, 3... 또는 Chapter 1, Chapter 2...)를 우선적으로 고려
   - preface, introduction, foreword, contents, index, acknowledgments 등은 제외

2. 각 장의 정확한 페이지 범위를 계산하세요:
   - 각 장의 시작 페이지: 해당 항목의 page 값
   - 각 장의 종료 페이지: 다음 장의 시작 페이지 - 1 (마지막 장은 목차의 마지막 페이지)

3. 식별된 장들의 정보를 정확히 제공하세요.

응답은 반드시 JSON 형식으로만 반환하세요:"""

        # JSON 템플릿을 별도 문자열로 추가
        json_template = """{
    "analysis": {
        "total_items_analyzed": 218,
        "identified_chapter_level": "level_1",
        "chapter_pattern": "Numbered chapters like '1_Complexity_of_object_oriented_programming'"
    },
    "chapters": [
        {
            "id": 15,
            "title": "1_Complexity_of_object_oriented_programming",
            "start_page": 31,
            "end_page": 53
        },
        {
            "id": 27,
            "title": "2_Separation_between_code_and_data",
            "start_page": 54,
            "end_page": 70
        }
    ],
    "total_chapters": 6
        ...
        logger.info(f"AI({ai_provider.get_name()})에게 목차 구조 분석 요청 중...")
        ...
        try:
            ...
            if 'chapters' in parsed_result and isinstance(parsed_result['chapters'], list):
                ...
                if 'analysis' in parsed_result:
                    ...
                    logger.info(f"식별된 장 레벨: {analysis.get('identified_chapter_level', 'N/A')}")
                    logger.info(f"장 패턴: {analysis.get('chapter_pattern', 'N/A')}")
                
                return {
                    'success': True,
                    'total_chapters': parsed_result.get('total_chapters', len(chapters_info)),
        ...
        logger.error(f"목차 구조 분석 실패: {str(e)}")
        return {
            'success': False,
            'error': str(e),
            'ai_provider': ai_provider.get_name() if ai_provider else "Unknown"
        }
def find_chapter_items(toc_structure: List[Dict], chapter_id: int, next_chapter_id: Optional[int], logger) -> List[Dict]:
    ...
    logger.info(f"장 ID: {chapter_id}, 다음 장 ID: {next_chapter_id}")
    
    # 해당 장의 시작 항목 찾기
    if chapter_id not in id_to_item:
        logger.error(f"장 ID {chapter_id}를 찾을 수 없습니다.")
        return []
    
    chapter_start_item = id_to_item[chapter_id]
    ...
    def collect_children(item_id: int):
        ...
        for child_id in children_ids:
            # 다음 장 시작점에 도달하면 중단
            if next_chapter_id and child_id == next_chapter_id:
    ...
    try:
        ...
        for i, chapter_info in enumerate(chapters_info):
            ...
            if i + 1 < len(chapters_info):
                next_chapter_id = chapters_info[i + 1]['id']
...
async def process_chapters(toc_json_path: str, config: Dict[str, Any], logger):
    """전체 처리 워크플로우: AI로 목차 구조 분석 → 프로그래밍적으로 목차 추출"""
    try:
        # 목차 파일 읽기
        logger.info(f"목차 파일 읽는 중: {toc_json_path}")
        with open(toc_json_path, 'r', encoding='utf-8') as f:
        ...
        # 1단계: AI로 목차 구조 분석
        logger.info("=== 1단계: 목차 구조 분석 시작 ===")
        analysis_result = await analyze_toc_structure_with_ai(toc_json_path, ai_provider, logger)
        
        if not analysis_result['success']:
            logger.error("❌ 목차 구조 분석 실패")
            return analysis_result
        
        chapters_info = analysis_result.get('chapters_info', [])
        chapter_titles = analysis_result['chapter_titles']
        logger.info(f"✅ {analysis_result['total_chapters']}개 장 발견")
        
        # 분석 결과 출력
        if 'analysis' in analysis_result and analysis_result['analysis']:
            analysis = analysis_result['analysis']
            logger.info(f"분석 결과:")
            logger.info(f"  - 분석된 항목 수: {analysis.get('total_items_analyzed', 'N/A')}")
            logger.info(f"  - 식별된 장 레벨: {analysis.get('identified_chapter_level', 'N/A')}")
            logger.info(f"  - 장 패턴: {analysis.get('chapter_pattern', 'N/A')}")
        
        if chapters_info:
            logger.info("AI가 식별한 장별 정보:")
        ...
        final_result = {
            'success': True,
            'ai_provider': ai_provider.get_name(),
            'total_chapters': analysis_result['total_chapters'],
            'chapter_titles': chapter_titles,
            'chapters_info': chapters_info,
...
async def main():
    ...
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("사용법: python extract_chapters_v4.py <목차JSON파일> <PDF파일> [config파일]")
        print("예시: python extract_chapters_v4.py toc.json book.pdf")
        print("예시: python extract_chapters_v4.py toc.json book.pdf config.yaml")
        print()
    ...
    # 로거 설정 (config 기반)
    logger = setup_logging("extract_chapters_v4", config)
    logger.info(f"=== 범용적 목차 구조 분석 및 장별 내용 추출 시작 (AI: {ai_provider_type.upper()}, Config: {config_file}) ===")
    ...
    if result['success']:
        ...
        if 'analysis' in result and result['analysis']:
            ...
            print(f"  - 분석된 항목 수: {analysis.get('total_items_analyzed', 'N/A')}")
            print(f"  - 식별된 장 레벨: {analysis.get('identified_chapter_level', 'N/A')}")
            print(f"  - 장 패턴: {analysis.get('chapter_pattern', 'N/A')}")
        ...
        # 결과를 파일로 저장
        result_file = f"extract_chapters_v4_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    logger.info("=== 범용적 목차 구조 분석 및 장별 내용 추출 완료 ===")
