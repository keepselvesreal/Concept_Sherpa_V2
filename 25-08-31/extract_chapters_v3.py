# 생성 시간: 2025-08-31 17:17:00 KST
# 핵심 내용: PDF 내용 추출이 포함된 장별 목차 및 내용 추출 모듈
# 상세 내용:
...
#   - count_chapters_with_ai (라인 195): AI를 통한 장 개수 및 페이지 정보 확인
...
async def count_chapters_with_ai(toc_json_path: str, ai_provider: AIProvider, logger) -> Dict[str, Any]:
    ...
    try:
        ...
        # AI에게 장 개수 확인 요청
        prompt = f"""다음 목차 데이터를 분석하여 총 몇 개의 장(chapter)이 있는지 확인하고, 각 장의 시작/종료 페이지 정보도 함께 제공해주세요.
...
2. 총 장의 개수를 계산하세요
3. 각 장의 제목, 시작 페이지, 종료 페이지를 나열하세요
...
{{
    ...
    "chapters": [
        ...
        try:
            ...
            # 새로운 형식 지원
            if 'chapters' in parsed_result:
                chapters_info = parsed_result.get('chapters', [])
...
def extract_chapters_programmatically(toc_structure: List[Dict], chapter_titles: List[str], chapters_info: List[Dict], logger, config: Dict[str, Any]):
    """프로그래밍적으로 각 장별 목차 추출"""
    try:
        # 출력 디렉토리 설정
        ...
        for i, chapter_title in enumerate(chapter_titles):
            ...
            if chapter_item:
                ...
                # AI에서 제공한 페이지 정보 추가
                if chapters_info and i < len(chapters_info):
                    chapter_info = chapters_info[i]
                    chapter_mapping['start_page'] = chapter_info.get('start_page')
                    chapter_mapping['end_page'] = chapter_info.get('end_page')
                    logger.info(f"장 {chapter_number}: {chapter_title} (ID: {chapter_item['id']}, 페이지: {chapter_info.get('start_page')}-{chapter_info.get('end_page')})")
                else:
                    logger.info(f"장 {chapter_number}: {chapter_title} (ID: {chapter_item['id']})")
                
                chapter_mappings.append(chapter_mapping)
            else:
        ...
        for i, chapter_info in enumerate(chapter_mappings):
            ...
            if chapter_items:
                ...
                chapter_content = ""
                if hasattr(chapter_info, 'get') and 'start_page' in chapter_info and 'end_page' in chapter_info:
                    pdf_path = config.get('pdf_path', '')
                    if pdf_path and Path(pdf_path).exists():
                        logger.info(f"장 {chapter_info['chapter_number']} PDF 내용 추출 시작: 페이지 {chapter_info['start_page']}-{chapter_info['end_page']}")
...
async def process_chapters(toc_json_path: str, config: Dict[str, Any], logger):
    ...
    try:
        ...
        extract_result = extract_chapters_programmatically(toc_structure, chapter_titles, chapters_info, logger, config)
...
async def main():
    ...
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("사용법: python extract_chapters_v3.py <목차JSON파일> <PDF파일> [config파일]")
        print("예시: python extract_chapters_v3.py toc.json book.pdf")
        print("예시: python extract_chapters_v3.py toc.json book.pdf config.yaml")
        print()
    ...
    # 로거 설정 (config 기반)
    logger = setup_logging("extract_chapters_v3", config)
    logger.info(f"=== PDF 내용 추출 포함 목차 추출 시작 (AI: {ai_provider_type.upper()}, Config: {config_file}) ===")
    ...
    if result['success']:
        ...
        # 결과를 파일로 저장
        result_file = f"extract_chapters_v3_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    logger.info("=== PDF 내용 추출 포함 목차 추출 완료 ===")
