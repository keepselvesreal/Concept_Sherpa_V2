# 생성 시간: 2025-08-31 17:10:08 KST
# 핵심 내용: 장별 폴더 생성 및 정규화된 목차 추출 모듈
# 상세 내용:
#   - normalize_title (라인 35): 제목 정규화 함수 (특수문자 제거 및 언더스코어 변환)
#   - setup_logging (라인 45): 로깅 설정 및 로그 파일 생성
#   - AIProvider (라인 65): AI 제공자 추상 베이스 클래스
#   - ClaudeSDKProvider (라인 85): Claude SDK 구현체
#   - GeminiAPIProvider (라인 125): Gemini API 구현체
#   - count_chapters_with_ai (라인 185): AI를 통한 장 개수 확인
#   - find_chapter_items (라인 245): 각 장에 속하는 목차 항목들을 찾는 함수
#   - save_chapter_toc_to_folder (라인 295): 장별 폴더에 정규화된 목차 저장
#   - extract_chapters_programmatically (라인 325): 프로그래밍적 목차 추출 (폴더 생성 포함)
#   - process_chapters (라인 415): 전체 처리 워크플로우
#   - main (라인 485): 메인 실행 함수
# 상태: active
# 주소: extract_chapters_v2
# 참조: extract_chapters
...
def save_chapter_toc_to_folder(chapter_title: str, chapter_items: List[Dict], output_dir: Path, logger):
    """장별 폴더에 정규화된 목차 저장"""
    # 장 제목 정규화
    ...
    # 목차 파일 저장
    filename = f"{normalized_chapter_title}_toc.json"
    filepath = chapter_dir / filename
    
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(normalized_items, f, ensure_ascii=False, indent=2)
    
    logger.info(f"장 '{chapter_title}' 목차 저장 완료: {filepath}")
    return filepath
...
def extract_chapters_programmatically(toc_structure: List[Dict], chapter_titles: List[str], logger, config: Dict[str, Any]):
    ...
    try:
        ...
        for i, chapter_info in enumerate(chapter_mappings):
            ...
            if chapter_items:
                # 장별 폴더에 정규화된 목차 저장
                saved_file = save_chapter_toc_to_folder(
                    chapter_info['title'], 
                    chapter_items, 
                    output_dir, 
                    logger
