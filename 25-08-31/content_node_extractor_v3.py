"""
생성 시간: 2025-08-31 20:58:50 KST
핵심 내용: AI 기반 콘텐츠 노드 추출 및 분석 시스템 (end_page 자동 수정 포함)
상세 내용:
    ...
    - ContentNodeExtractor 클래스 (132-410): AI 기반 콘텐츠 노드 추출 로직
        ...
        - update_end_pages (350-375): end_page 자동 수정 로직
        - analyze_content_with_ai (300-340): AI를 통한 콘텐츠 존재 여부 분석
        - process_and_extract (380-410): 전체 처리 및 콘텐츠 노드 추출
    - main 함수 (412-445): 스크립트 실행 진입점 (ConfigManager 통합)
상태: active
주소: content_node_extractor_v3
참조: content_node_extractor_v2, config_manager, update_end_pages
"""
...
{{
    ...
    def update_end_pages(self, nodes):
        """
        노드 리스트에서 end_page를 자동으로 수정
        다음 항목의 level이 현재 항목보다 큰 경우 end_page를 다음 항목의 start_page로 수정
        """
        updated_count = 0
        print("\n=== end_page 자동 수정 프로세스 시작 ===")
        ...
        for i in range(len(nodes)):
            ...
            if i + 1 < len(nodes):
                ...
                if next_node['level'] > current_node['level']:
                    ...
                    print(f"🔄 [{current_node['title']}] end_page: {old_end_page} → {new_end_page}")
                    updated_count += 1
        
        print(f"✅ end_page 수정 완료: {updated_count}개 항목 업데이트")
        return nodes
    ...
    async def process_and_extract(self, json_path, markdown_path, output_dir="."):
        ...
        # 5단계: end_page 자동 수정
        content_nodes = self.update_end_pages(content_nodes)
