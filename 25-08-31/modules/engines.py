...
핵심 내용을 2-3문장으로 요약해주세요."""
        ...
        if update_logger:
            await update_logger.log_extraction_with_prompt(
                title, "핵심내용", prompt, system_prompt, response
            )
...
주요 개념과 중요한 세부사항을 포함하여 5-7문장으로 정리해주세요."""
        ...
        if update_logger:
            await update_logger.log_extraction_with_prompt(
                title, "상세핵심내용", prompt, system_prompt, response
            )
...
문서의 모든 중요한 정보를 빠뜨리지 않고 체계적으로 정리해주세요."""
        ...
        if update_logger:
            await update_logger.log_extraction_with_prompt(
                title, "상세정보", prompt, system_prompt, response
            )
...
문서에서 다루는 핵심 주제들을 불렛 포인트로 나열해주세요."""
        ...
        if update_logger:
            await update_logger.log_extraction_with_prompt(
                title, "주요화제", prompt, system_prompt, response
            )
...
주요 주제 외에 언급되는 부차적인 주제들을 불렛 포인트로 나열해주세요."""
        ...
        if update_logger:
            await update_logger.log_extraction_with_prompt(
                title, "부차화제", prompt, system_prompt, response
            )
